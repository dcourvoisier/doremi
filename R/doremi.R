...
#'
#' \code{doremi_analyse_order1}  the function performs a multilevel regression of a first order linear
...
#' @param userdata Is a data frame or data table containing at least one column, that is the signal to be analysed.
...
doremi_analyse_order1 <- function(userdata,
  ...
  if(!is.character(signal)){stop("signal should be a string containing the name of the column in data that contains the signal of the individual.\n")}
  ...
  if (!is.null(id) && is.character(data[[id]])){
    data[, paste0(id,"_tmp") := rep(seq(unique(get(id))), data[, .N, by = id]$N)]
  ...
  data <- data[!is.na(get(time)) & !is.na(get(signal))]
  ...
  if (!noinput){
    ...
    data[, (paste0(input,"_rollmean")) := lapply(.SD, myfun), .SDcols = input, by = id]
  ...
  if (!is.null(id)){ #If the id column is not null then it is assumed that there are SEVERAL INDIVidUALS
    ...
    if (!inherits(model,"error")){ # if the regression worked
      ...
      resultid[, c(paste0(signal, "_dampingtime")) := -1L/(summary$coefficients[paste0(signal, "_rollmean"), "Estimate"] + random[.GRP,paste0(signal, "_rollmean")]), by = id]

      # Extract the intercept (equilibrium value) calculated for each individual (present in random, regression table)
      # Offset in resultid ------------------------------------------------------
      resultid[, c(paste0(signal,"_eqvalue")) := (summary$coefficients["(Intercept)", "Estimate"] + random[.GRP, "(Intercept)"])*resultid[.GRP, get(paste0(signal, "_dampingtime"))], by = id]
      ...
      if(noinput){
        ...
        data[, expfit_A := lm(log(get(signal) - resultid[.GRP, get(paste0(signal, "_eqvalue"))]) ~ get(time))$coefficients[1], by = id]
      }
        ...
        for (i in 1:length(input)){ #For loop to go through all the inputs
          ...
          resultid[, c(paste0(input[i],"_exccoeff")) :=
                     (summary$coefficients[paste0(input[i], "_rollmean"), "Estimate"] + random[.GRP,paste0(input[i], "_rollmean")]) * resultid[.GRP, get(paste0(signal, "_dampingtime"))], by = id]
          data[, totalexc := totalexc+(summary$coefficients[paste0(input[i], "_rollmean"), "Estimate"] + random[.GRP,paste0(input[i], "_rollmean")]) * get(input[i]), by = id]
        }
      }

      # Write any error message from the regression
      resultmean[, c(paste0(signal,"_fitmsg")) := summary$fitMsgs]

      # Write a warning if any of the damping times calculated was negative
      if (any(is.na(resultid[, get(paste0(signal,"_dampingtime"))])) | any(resultid[, get(paste0(signal,"_dampingtime"))] < 0)){
      ...
      if (noinput){ #There is no excitation signal as input
        data[, c(paste0(signal,"_estimated")) :=
               if(!is.na(resultid[.GRP, get(paste0(signal,"_dampingtime"))]) && resultid[.GRP, get(paste0(signal,"_dampingtime"))] > 0){
                 ...
                 exp(expfit_A) * exp(-1L / resultid[.GRP, get(paste0(signal,"_dampingtime"))] * get(time)) + resultid[.GRP, get(paste0(signal,"_eqvalue"))]

               }else{NaN}, by = id]

        #Adding calculated columns to table "estimated" and removing them from table "data"
        estimated <- data[, c(oldid, time, paste0(signal,"_estimated")), with = FALSE]
        data <- data[, c("expfit_A", paste0(signal,"_estimated")) := NULL]
        ...
        estimated <- data[, list(timecol = seq(floor(min(get(time), na.rm = T)), ceiling(max(get(time), na.rm = T)), deltat)), by = oldid]

        #Finding the values of the original time vector in the expanded time vector by using the function "findInterval"
        IDvec <- unique(data[[oldid]])
        ...
        for (idx in seq(IDvec)){ #It is necessary to do a loop because findInterval finds the index in which the value is found in the original time vector
          ...
          leftidx <- findInterval(data[get(oldid) == IDvec[idx], get(time)], estimated[get(oldid) == IDvec[idx], timecol])
          ...
          estimated[nrow(estimated[get(oldid) < IDvec[idx]]) + leftidx, tmpsignal := data[get(oldid) == IDvec[idx], totalexc]]
        }
        ...
        estimated[, ymin := doremi_generate_order1(resultid[.GRP, get(paste0(signal,"_dampingtime"))],exc_min,timecol)$y+
                     resultid[.GRP, get(paste0(signal,"_eqvalue"))], by = oldid]
      ...
      resultid[, c(paste0(signal,c("_dampingtime","_exccoeff","_eqvalue"))) := NA]
      resultmean[, c(paste0(signal,c("_dampingtime","_exccoeff","_eqvalue"))) := NA]
      regression <- model
      estimated <- NULL
    }
    ...
    res = list(data = data, resultid = resultid, resultmean = resultmean, regression = regression, estimated = estimated, str_time = time, str_exc = str_exc, str_signal = signal, str_id = oldid)
  ...
  # Regression for SINGLE INDIVidUAL ----------------------------------------
    ...
    if (!inherits(model,"error")){ # if the regression worked
      ...
      if (!noinput){ # If there is an excitation term
        ...
        {
          resultmean[, c(paste0(input[i], "_exccoeff")) := summary$coefficients[paste0(input[i], "_rollmean"), "Estimate"] * resultmean[, get(paste0(signal, "_dampingtime"))]]
          data[, totalexc:= totalexc + (summary$coefficients[paste0(input[i], "_rollmean"), "Estimate"]) * get(input[i])]
        }
      }
      # Write any error message from the regression
      resultmean[, c(paste0(signal, "_fitmsg")) := summary$fitMsgs ]
      ...
      if (noinput){ #there is NO excitation as input
        ...
        #Adding calculated columns to table "estimated" and removing them from table "data"
        estimated <- data[, c(id, time, paste0(signal,"_estimated")), with = FALSE]
        data <- data[, c(paste0(signal,"_estimated")) := NULL]
        ...
        deltat <- 0.1
        #Expanded time vector: takes the minimum and the maximum of all the time intervals and creates the vector with deltat time intervals
        estimated <- data[, list(timecol = seq(floor(min(get(time), na.rm = T)), ceiling(max(get(time), na.rm = T)), deltat))]

        leftidx <- findInterval(data[, get(time)], estimated[, timecol])
        #tmpsignal contains the value of exctotal were the original times are found in the new time vector
        #and NA in the rest of the positions
        estimated[leftidx, tmpsignal := data[, totalexc]]
        ...
        #Calculating the convolution
        estimated[, ymin := doremi_generate_order1(resultmean[, get(paste0(signal, "_dampingtime"))],exc_min,timecol)$y+
                    resultmean[, get(paste0(signal,"_eqvalue"))]]
        estimated[, ymax := doremi_generate_order1(resultmean[, get(paste0(signal, "_dampingtime"))],exc_max,timecol)$y+
                    resultmean[, get(paste0(signal,"_eqvalue"))]]
    ...
    res = list(data = data, resultmean = resultmean, regression = summary, estimated = estimated, str_time = time, str_exc = str_exc, str_signal = signal, str_id = oldid)
...
#'
#' \code{doremi_generate_order1} returns a vector containing the convolution of the Green function
...
#'    \item rawdata- contains the signal and time values of the pseudo-continuous function with the parameters specified.
#' }
#' Each one of these elements contains the following columns:
#' \itemize{
#'    \item id - individual identifier (starting by 1 and increasing by 1).
#'    \item excitation - excitation signal generate through the excitation_function

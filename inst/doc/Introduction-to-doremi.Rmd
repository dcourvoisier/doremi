...
Now let's take a look to the different parts of the result. The analysis function supplies an object that contins several lists. This object has been given the attribute or class "doremi" in order to be able to create S3 methods for it as we will see further below.
  
As it was mentioned before, the first table of the doremi object generated contains the original data with some columns added. These columns contain intermediate variables used for the preparation of the regression:
...
Where we have a summary of the random and fixed effects and the residuals calculated by the function lmer. Finally, the reconstructed signal values can be visualized with:

```{r}
resultb1$estimated
```
    
If we graphically wish to verify how the estimated signal fits the initial signal, we can call the generic function "plot", as an S3 method has been added to it in order to handle doremi objects:

```{r fig.width = 7, fig.height = 6}
plot(resultb1)
```

Similarly to the _print_ function, _plot_ applied to a doremi object plots by default the first six individuals contained in the result. If we wish to visualize a single individual or a specific set of individuals, we can specify them by changing the "id" input parameter of the function:
...
And as it can be seen, the doremi plot generated contains the name of the doremi object variable in the title for future refernce, the legend with the variables represented and the labelling of the axis according to the variables introduced, all automatically.

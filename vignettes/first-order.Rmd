...
The coefficients of the diferential equation can thus be calculated as:
...
```
## Example 1b - Changing initial condition, gain and equilibrium value
In this example we show the flexibility of the functions to add changes on the mentioned parameters. For this, we simplify the form of the exitation to a simple pulse:
...
And it can be observed how the system tends to equilibrum (2) from the initial condition (3) then to a new equilibrium after being perturbed by the excitation (saturation at $yeq + ku(t)=2 + 5*1=7$) and then returns to equilibrium after the excitation is gone.

# Delphine: why is equilibrium lower than previous values 
...
# Delphine: is this zeroth order really necessary or could we say something simpler
...
plot(res2a)
```
# Delphine: it seems strange to use the plot function to generate data. I suggested a modification of the sentence just before the call to plot
...
# Delphine: j'ai une dizaine de message de warning comme ci-dessous:
#length(LHS)==0; no columns to delete or assign RHS to.WARN [2020-12-27 12:30:52] Excitation signal  is constant and is removed.
# Puis j'obtiens 11 et non 7
And can be seen that from the range provided, an embedding number of 7 produces the best fit and the coefficients are closer to their true values than the ones estimated in the previous example.
...
res7 <- analyze.1order(data = data5rd,
                       ...
                       signal = "signal")
```
# message de warning à nouveau
#length(LHS)==0; no columns to delete or assign RHS to.WARN [2020-12-27 12:37:30] Excitation signal  is constant and is removed.

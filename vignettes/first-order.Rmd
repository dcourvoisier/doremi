...
To generate the excitation process perturbing the system we want to simulate, wa can use the `generate.excitation` function, which generate a sequence of squarre pulses of a given length randomly distributed:

```{r}
set.seed(123)
U <- generate.excitation(nexc = 3, # number of squarre pulses
                           duration = 10, # pulse duration
                           deltatf = 1, # time spacing between points
                           tmax = 100, # maximum time
                           minspacing = 20) # minimum spacin between squarre pulses
plot(U$t,U$exc)
```
...
And it can be observed how the system tends to equilibrum ($t_{eq} = 2$) from the initial condition ($y_0 = 3$) after being perturbed by the excitations.
...
```{r}
ggplot2::ggplot(cardio[id == 2],aes(x = time))+
  geom_point(aes(y = hr,color = "heart rate (beat/s)"))+
  geom_line(aes(y = load,color = "effort load (W)"))
```

We can perfrom the analysis using `analyze.1order`:
...
And use the `plot` function to plott the analysis results for all 21 participants. Note that omitting the argument `id` would lead to only the first 6 participants being plotted by default.
...
Such analysis has been used in [(Mongin et al., 2020)](https://doi.org/10.1080/00273171.2020.1754155) and [Mongin et al., 2020](https://doi.org/10.1038/s41598-020-69218-1).
...
This analysis has been the main one in [Mongin et al.](https://doi.org/10.1088/1361-6579/abbb6e) and used in [Mongin et al.](https://doi.org/10.1109/ESGCO49734.2020.9158156).

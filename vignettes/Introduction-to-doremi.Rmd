...
In order to find these coefficients, the model performs the following linear mixed-effect regression derived from equation (1), as presented by [(Mongin et al., 2020, doi: 10.1080/00273171.2020.1754155)]:
...
With the estimated parameters, the signal can be reconstructed for each individual since the analytical solution to equation (1) is known:
...
These are functions that allow the user to create data corresponding to a first order differential equation (i.e. solution of equation (1)). The simulation functions presented are:


* **generate.remi:** creates the solution of equation (1) for a given damping time and excitation vector.


* **generate.panel.remi:** creates a set of signals that are a solution of equation (1) for random simulated excitations. Number of points, damping time, intra-individual noise and inter-individual noise can be controlled.
...
# Delphine: quand je lance le code ci-dessus, j'obtiens d'abord un message qui indique que
# je n'ai pas la library data.table ("could not find setDT)
# puis quand je charge la library, j'ai quand même: "Error: attempt to use zero-length variable name"
...
Analyzing the data generated in the example 2 above, the user must specify the name of the columns containing the id of the participants, the excitation, and the signal. In addition, the user must also indicate the embedding number. The embedding number is the number of data points used to compute the first derivative using the GOLD method (see the package manual for more details).
...
Beware that the standard errors coming for this regression are not entirely correct as the calculation of derivatives wil "smooth" the fit and underestimate them. Please see (Mongin et al., 2020, doi: 10.1080/00273171.2020.1754155.) for more details.
...
It can be observed that all three estimated coefficients are close to the values given to the simulation function (10 for damping time, 0 for the equilibrium value, and 10 for the excitation coefficient since the simulated signal had an excitation coefficient equal to $\tau$). And for each individual we have:
...
And, taking into account the form of the analytical solution:
$$y(t) = \frac{\epsilon}{\tau}\int E(t')G(t-t')dt'+ eqvalue$$
One can find the excitation coefficients introduced either by extracting them from the summary of the regression or by taking the values from the `$resultid` table and dividing them by the damping time as explained above: $\epsilon_{1}=10.32$, $\epsilon_{2}=37.52$, $\epsilon_{3}=44.79$ for individual 1 and
...
Similarly to the example 2, we will generate data so that the analysis can fit a decreasing exponential. Fitting a decreasing exponential is appropriate when there is either no excitation, when the excitation is unknown, or when it is constant over time.  

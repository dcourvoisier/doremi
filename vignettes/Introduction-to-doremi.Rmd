...
The detail of the analysis when supposing that the studied variable follows a first or second order differential equation is detailed in the associated vignettes for the [first](first-order.html) or [second](sedond-order.html). The functions used to calculate the derivatives are detailed in the [derivatives](derivatives.html) vignette.

# EXAMPLES {#example}

Two examples of analysis are shown below. Detailed use of the functions can be found in the vignettes for the analyss based on a [first](first-order.html) or [second](sedond-order.html) order differential equation.
...
The same can be performed for a noisy signal following a second order differential equation driven by the same excitation. Let us generate noisy data driven by the excitation and governed bty a seocnd order differential equation:
...
We can use `generate.2order` to generate a solution of this differential equation, for a given $U(t)$, a period of 15, and a daming factor of 0.2:

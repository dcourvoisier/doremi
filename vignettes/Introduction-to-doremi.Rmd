...
The first step consist in the estimation of the derivatives, and the second one of the estimation of the differential equation coefficient thanks to a linear mixed-effects (multilevel) models. 
...
derivative_example <- data.table(expand.grid(list(time = timevec,
                                                  embedding = 3:10,
...
We know the first derivative is $\dot{y} = 2t$ and that the second derivative is $\ddot{y} = 2$. Lets us apply our three functions to estimate the zeroth, first and second order derivative (the zeroth order derivative is a smooth estimate of the variable) and compare with the true derivatives.

```{r}
derivative_example[,spar :=( embedding-3 )/7]
# calculation of the derivatives with calculate.gold
...
variable<- generate.1order(time = time,
                         ...
                         k = 1,
                         yeq = 0)
variable<-cbind(data.table::as.data.table(variable),exc)
...
* $k$ is the gain.  It indicates the proportionality between the maximal amplitude the variable can reach and the excitation.
...
Once the derivative estimated, doremi performs the following linear mixed-effect regression derived from equation \ref{1}:

\begin{equation}
\dot{y}_{ij} \sim b_{0}+b_{0j}+b_{1}y_{ij}+b_{2}U_{ij}+u_{1j}y_{ij}+u_{2j}U_{ij}+e_{ij} 
\label{eq2}
\end{equation}
...
Note that random effects are estimated for the intercept ($b_{0} + b_{0j}$), variable ($b_{1} + u_{1j}$) and excitation terms ($b_{2} + u_{2j}$), so that individuals can have different coefficients (initial condition, damping time, gain and equilibrium value).
...
With the above estimated parameters, the estimated variable can be reconstructed for each individual using the numerical estimation provided by the function `ode` from the package deSolve.
...
* $y_{eq}$ is the variable equilibrium value, i.e. the value reached when the excitation term is 0 or constant.

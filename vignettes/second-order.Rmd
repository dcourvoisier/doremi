...
$$\frac{d^2y}{dt} + 2\xi\omega_{n}\frac{dy}{dt} + \omega_{n}^2 y = k*u(t)$$ 
(1)

In which the term $u(t)=0$ thus leading to the equation:
$$y'' + 2\xi\omega_{n}y' + \omega_{n}^2 y = 0$$ 
(2)
...
```{r}
res11a <- rbindlist(lapply(seq(0,2,0.2), function(eps){
  generate.2order(time = 0:49, y0 = 1, xi = eps, period = 30)[,xi := eps][]
}))
# plot
ggplot2::ggplot(res11a,ggplot2::aes(t,y,color = as.factor(xi)))+
  ggplot2::geom_line() +
  ggplot2::labs(x = "time (arb. unit)", y = "signal (arb. unit)", colour = "xi")
```

Generating Damped Linear Oscillator signals with inter-individual variability:
```{r}
time <- 0:100
res11b <- generate.panel.2order(time = time,
                               y0 = 10,
                               v0 = 0,
                               xi = 0.1,
                               period = 30,
...
Similarly as for first order differential equation models, in this section we present increasingly complex examples of data simulation for the second order differential equation model functions included in the package.

## Example 12 - Generating signals with no noise

In this example we will generate data for 5 individuals, with a damping time of 10s for an excitation vector formed by 3 pulses of amplitude 1 and duration 10s distributed randomly in a time period of 100s, with a minimum spacing between pulses of 20 s and with no noise. That is, the signal follows exactly the differential equation (no dynamic noise) with no variation of the damping rate, the period and the equilibrium value across individuals (no interindividual noise). 
```{r}
time <- 0:100
res12 <- generate.panel.2order(time = time,
...
In the same way, the function returns a data.table with the class 'doremidata'. The object is the same as the result form the generate.panel.1order function in the case of first order differential equations (see the corresponding vignette for more details).
...
## Example 13 - Generating signals with noise
...
time <- 0:100
res13 <- generate.panel.2order(time = time,
                               excitation = as.numeric(time>20),
                               y0 = 0,
                               v0 = 0,
                               xi = 0.1,
...
## Example 14 - including the effect of periodical excitations
...
## Using an initial condition in a time different from t0=0
